"""Fine-tune bert-base-uncased on train.csv for multi-label toxicity
classification, and compare its validation AUC against the no-fine-tuning
baseline in evaluate_base.py.

Uses the same VALIDATION_SIZE/RANDOM_SEED as evaluate_base.py and
evaluate_detoxify.py, so train_test_split produces the identical held-out
rows -- results are directly comparable across all three scripts.
"""
